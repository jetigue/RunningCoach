<template>
    <div class="">
        <div v-if="editing" class="p-3 border-b border-blue-lighter">
            <div class="w-full">
                <form action="api/run-feelings/id" method="POST" id="editDayTime" @submit.prevent="update"
...
<script>
    export default {
        ...
        methods: {
            ...
            update() {
                this.form
                    .patch('/api/run-feelings/' + this.data.id)
                    .then(data => {
            ...
            destroy() {
                axios.delete('api/run-feelings/' + this.data.id);
