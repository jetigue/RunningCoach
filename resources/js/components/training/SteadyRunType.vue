<template>
    <div class="">
        <div v-if="editing" class="p-3 border-b border-blue-lighter">
            <div class="w-full">
                <form action="api/steady-run-types/id" method="POST" id="editSteadyRun" @submit.prevent="update"
...
<script>
    export default {
        ...
        methods: {
            ...
            update() {
                this.form
                    .patch('/api/steady-run-types/' + this.data.id)
                    .then(data => {
            ...
            destroy() {
                axios.delete('api/steady-run-types/' + this.data.id);
